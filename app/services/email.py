...
RESEND_FROM = os.getenv("RESEND_FROM", "Cogniva <onboarding@resend.dev>")
...
def send_email(subject: str, body_text: str, body_html: str = None, recipients: list = None):
    if recipients is None:
        ...
        try:
            ...
            recipients.extend(db_subscribers)
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"Could not fetch subscribers from DB: {e}")
        
        # Also include MY_EMAIL as fallback/admin
    ...
    recipients = [r.strip() for r in recipients if r is not None]
    if not recipients:
